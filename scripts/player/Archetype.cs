    ...
    public static void ApplyTo(Archetype archetype, PlayerStats stats)
    {
        switch (archetype)
        {
            case Archetype.Fighter:
                stats.SetBaseStats(maxHp: 40, attackDamage: 5, moveSpeed: 5.0f, attackRange: 1.25f);
                break;
            case Archetype.Archer:
                stats.SetBaseStats(maxHp: 24, attackDamage: 3, moveSpeed: 6.0f, attackRange: 1.45f);
                break;
            case Archetype.Mage:
                stats.SetBaseStats(maxHp: 20, attackDamage: 6, moveSpeed: 4.5f, attackRange: 1.35f);
                break;
        }
        stats.ResetHp();
    }
