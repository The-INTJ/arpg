    ...
    public static void ApplyTo(Archetype archetype, PlayerStats stats)
    {
        switch (archetype)
        {
            case Archetype.Fighter:
                stats.SetBaseStats(maxHp: 20, attackDamage: 5, moveSpeed: 5.0f, attackRange: 3.0f);
                break;
            case Archetype.Archer:
                stats.SetBaseStats(maxHp: 12, attackDamage: 3, moveSpeed: 6.0f, attackRange: 6.0f);
                break;
            case Archetype.Mage:
                stats.SetBaseStats(maxHp: 10, attackDamage: 6, moveSpeed: 4.5f, attackRange: 5.0f);
                break;
        }
        stats.ResetHp();
    }
